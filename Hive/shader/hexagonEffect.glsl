...
uniform highp float flatSide;
uniform int mask;
uniform int strokeMask;
...
void main() {
    ...
    if(mask == 1) _color = texture2D(source, hpos/s.yx*ratio.yx * max(s.y,s.x) - 0.025);
    if(strokeMask == 1) _strokeColor = texture2D(strokeSource, hpos/s.yx * ratio.yx * max(s.y,s.x) - 0.02);
